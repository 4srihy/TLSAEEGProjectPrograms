...
if isfield(displaySettings,'displaySignificanceColor')
    displaySignificanceColor = displaySettings.displaySignificanceColor;
else
    displaySignificanceColor = [0 0 0; 0 1 0];
end
...
%            end
       ...
       elseif bonferroniFlag==2 %cluster correction
           if (p05(i)~=0)
               patch(xVals,yVals,displaySignificanceColor(1,:),'linestyle','none');
           end
           if (p01(i)~=0)
               patch(xVals,yVals,displaySignificanceColor(2,:),'linestyle','none');
           end
       else
           if (p(i)<0.05)
               patch(xVals,yVals,displaySignificanceColor(1,:),'linestyle','none');
