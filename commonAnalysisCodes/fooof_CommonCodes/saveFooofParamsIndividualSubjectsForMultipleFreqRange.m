...
function saveFooofParamsIndividualSubjectsForMultipleFreqRange(folderSourceString,subjectName,projectName,refType,protocolType, powerType, knee_flag,fooofPlotFlag,rmsPlotFlag,noneElec,fooofFlag)
...
switch projectName
    ...
    case 'MeditationProject'
        load(fullfile(foldername,protocolType,[subjectName '_' refType '_stRange_250_1250_v8_b30d5.mat']));

        if ~(strcmp(protocolType,'G1') || strcmp(protocolType,'G2') || strcmp(protocolType,'M2'))
            blPowerVsFreqTopo(:,:,2) = stPowerVsFreqTopo;  % merging all trials
        end
end
...
allFreqRangeWidths = [76];%[40 116 300];%[100 76  200];%[100 116 300];%[40] %[100]
if allFreqRangeWidths(1)==100
    centreFreq = 40:20:960;
else
    centreFreq =20:10:450;%20:10:950;
    ...
    %         fooof_ind.peak_params
     ...
     switch projectName
        case 'ADGammaProject'
         ...
         case 'MeditationProject'
             saveFolder = fullfile(folderSourceString,projectName,protocolType,'FOOOF'); makeDirectory(saveFolder);
     end
     if fooofFlag
         if knee_flag
             saveFileName = fullfile(saveFolder,[subjectName '_freqWidth_' num2str(freqRangeWidth) '_' refType '_' powerType 'withKnee.mat']);
         else
             saveFileName = fullfile(saveFolder,[subjectName '_freqWidth_' num2str(freqRangeWidth) '_' refType '_' powerType 'withoutKnee.mat']);
         end

     else
         saveFileName = fullfile(saveFolder,[subjectName '_freqWidth_' num2str(freqRangeWidth) '_' refType '_' powerType 'withoutFooof.mat']);
     end
